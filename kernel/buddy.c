...
bd_malloc(uint64 nbytes)
{
  ...
  // 修改：bit_set(bd_sizes[k].alloc, blk_index(k, p));
  ...
  for(; k > fk; k--) {
    ...
    // 修改：bit_set(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    // 修改：bit_clear(bd_sizes[k].alloc, bi);  // free p at size k
    bit_toggle(bd_sizes[k].alloc, bi);
    // 修改：if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      // 修改：bit_set(bd_sizes[k].alloc, bi);
      bit_toggle(bd_sizes[k].alloc,bi);
    }
  }
}
...
bd_initfree_pair(int k, int bi,void *left,void *right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  //修改：if(bit_isset(bd_sizes[k].alloc, bi) !=  bit_isset(bd_sizes[k].alloc, buddy)) {
  if(bit_isset(bd_sizes[k].alloc,bi/2)){
    // one of the pair is free
    free = BLK_SIZE(k);
    // 修改：if(bit_isset(bd_sizes[k].alloc, bi))
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    //修改：free += bd_initfree_pair(k, left);
    free += bd_initfree_pair(k, left,bd_left,bd_right);
    if(right <= left)
      continue;
    //修改：free += bd_initfree_pair(k, right);
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    //修改：sz = sizeof(char)* ROUNDUP(NBLK(k), 8)/8;
